...
const Header = () => {
          ...
          >
            <div className="pl-2 mt-1 space-y-1">
              {item.submenu.map((sub) => (
                <MobileMenuItem key={sub.id} item={sub} depth={depth + 1} />
              ))}
            </div>
          </div>
        )}
  ...
  return (
    <>
      ...
      <header
        className={`fixed top-9 md:top-12 left-0 right-0 z-40 transition-all duration-300 ${
          scrolled
                  ...
                  {isDropdownOpen && (
                    <div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden z-50 animate-in fade-in-0 zoom-in-95 duration-200">
                      <div className="py-2">
                        ...
                        <button
                          onClick={() => {
                            navigate("/createPuskesmasPembantu");
                            setIsDropdownOpen(false);
                          }}
            ...
            >
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-6 h-6 relative">
                  ...
                  ></span>
                  <span
                    className={`absolute top-1/2 left-0 w-6 h-0.5 bg-white transform -translate-y-0.5 transition-all duration-300 ${
                      menuOpen ? "opacity-0" : ""
                    }`}
                  ></span>
                  <span
                    className={`absolute bottom-2 left-0 w-6 h-0.5 bg-white transform transition-all duration-300 ${
                      menuOpen ? "-rotate-45 -translate-y-1.5" : ""
                    }`}
                  ></span>
                </div>
      ...
      >
        ...
        ></div>
        <nav
          className={`absolute top-4 left-4 right-4 bg-[#355F2E] rounded-2xl shadow-2xl border border-white/10 overflow-hidden transform transition-all duration-300 max-h-[calc(100vh-120px)] ${
